...
const CardHistoric: React.FC<{ historic: Order }> = ({ historic }) => {
			...
			<Card.Body>
				...
				<Row className="ps-2">
					...
					<Col xs={5} className="p-0">
						<ul className="fw-bolder p-0">
							{historic.product.creams.map((cream) => (
					...
					<Row className="ps-2">
						...
						<Col xs={5} className="p-0">
							<ul className="fw-bolder text-white p-0">
								{historic.product.toppings.map((topping) => (
									<ol key={topping.id} className="p-0 text-white text-truncate">
										{topping.name}
									</ol>
								))}
							</ul>
				...
				{historic.product.extras && (
					<Row className="ps-2">
						...
						<Col xs={5} className="p-0">
							<ul className="fw-bolder p-0">
								{historic.product.extras.map((extras) => (
									<ol key={extras.id} className="p-0 text-white text-truncate">
										{extras.name}
									</ol>
								))}
							</ul>
				...
				<Row className="d-flex">
					...
					<Col xs={5} className="p-0 fw-bolder">
						<span className="text-white">
							R${" "}
							{historic.product.value.toLocaleString("pt-BR", {
								currency: "BRL",
